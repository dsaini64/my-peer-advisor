...
with open('html/history.html', 'r') as webpage:
...
for tag in soup.find_all('h3'):
    if tag.get_text() == "11A. and 12A. Cultures & Ideas I and II":
        firstCodeAndName = tag
        break

for tag in soup.find_all('p'):
    if "A two-course sequence focusing on a major theme in human experience and culture over a significant" in tag.get_text():
...
# temp = sm.add_course_code_and_name(firstCodeAndName.get_text(), 'CHST')
...
#         print(tag)
#     print('\n')
#     temp = sm.add_course_code_and_name(tagCodeAndName.get_text(), 'CHST')
